...
import { Shield, Eye, EyeOff, Mail, Phone, Lock, ArrowRight, MessageSquare, CheckCircle2 } from 'lucide-react';
...
export default function LoginPage() {
  ...
  const handleSendOtp = async (e: React.FormEvent) => {
    ...
    try {
      await fetchApi('/auth/send-phone-otp', {
        method: 'POST',
        body: JSON.stringify({ phoneNumber: otpPhone.trim() }),
      });
      setOtpSent(true);
      toast.success('Verification code sent to your WhatsApp!');
    } catch (err: any) {
      toast.error(err.message || 'Failed to send WhatsApp code');
    } finally {
      ...
      <main className="flex-grow flex items-center justify-center p-4 py-12 relative overflow-hidden">
        ...
        <div className="w-full max-w-md glass p-8 rounded-3xl border border-dark-800 shadow-2xl relative z-10">
          <div className="text-center mb-6">
            ...
            <p className="text-sm text-dark-400">Sign in to your GiftVault account</p>
          </div>

          {/* Auth Mode Toggle */}
          <div className="grid grid-cols-2 p-1 bg-dark-900/80 border border-dark-800 rounded-xl mb-6">
            <button
              type="button"
              ...
              {!otpSent ? (
                <form onSubmit={handleSendOtp} className="space-y-5">
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        required
                        className="input-field pl-11 py-3 text-sm rounded-xl bg-dark-900/80 border-dark-700 focus:border-emerald-500 text-white placeholder-dark-500 w-full"
                        placeholder="+9613794986"
                      />
                    </div>
                    <p className="text-[11px] text-dark-400 mt-1.5">
                      We'll send a 6-digit verification code directly to your WhatsApp via CallMeBot.
                    </p>
                  </div>
                  ...
                  <button
                    ...
                    className="w-full py-3.5 rounded-xl font-semibold bg-emerald-600 hover:bg-emerald-500 text-white flex items-center justify-center gap-2 text-sm shadow-glow-sm transition-all disabled:opacity-50"
                  >
                    {isSendingOtp ? (
                ...
                <form onSubmit={handleVerifyOtp} className="space-y-5">
                  <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-xl p-3 flex items-center gap-2 text-emerald-300 text-xs">
                    ...
                    <span>Verification code sent to {otpPhone}</span>
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      required
                      className="input-field py-3 text-center tracking-[0.5em] text-lg font-bold rounded-xl bg-dark-900/80 border-dark-700 focus:border-emerald-500 text-white placeholder-dark-500 w-full"
                      placeholder="123456"
                    />
                  </div>
                  ...
                  <button
                    ...
                    className="w-full text-xs text-dark-400 hover:text-white transition-colors text-center"
                  >
