...
export function Navbar() {
  ...
  return (
    <nav className="sticky top-0 z-50 w-full glass-strong border-b-0 border-white/5 shadow-md">
        ...
        <div className="flex items-center justify-between h-20">
          ...
          <div className="hidden md:flex items-center gap-6">
            ...
            <div className="relative group">
              ...
              <input 
                ...
                className="pl-10 pr-4 py-2 w-64 bg-dark-900/50 border border-dark-700 rounded-full text-sm text-white focus:outline-none focus:border-primary-500 focus:ring-1 focus:ring-primary-500/50 transition-all"
              />
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 md:gap-5">
            ...
            >
              ...
              {itemCount > 0 && (
                <span className="absolute top-0 right-0 w-5 h-5 bg-gradient-to-r from-primary-500 to-secondary-500 rounded-full flex items-center justify-center text-[10px] font-bold text-white shadow-glow-sm">
                  {itemCount}
                </span>
              )}
            </button>
            ...
            <div className="hidden md:flex items-center gap-4 border-l border-dark-700 pl-5">
              {isAuthenticated ? (
                <div className="relative group">
                  ...
                  <div className="absolute right-0 top-full mt-2 w-48 glass border border-dark-700 rounded-xl shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 transform origin-top-right">
      ...
      {/* Mobile Menu */}
      {isMobileMenuOpen && (
        <div className="md:hidden absolute top-20 left-0 w-full glass-strong border-t border-dark-700 animate-fade-in-down shadow-xl">
          <div className="p-4 space-y-4">
            <div className="relative">
              ...
              <input 
                type="text" 
                placeholder={t('common.search')}
                className="w-full pl-10 pr-4 py-3 bg-dark-900/50 border border-dark-700 rounded-xl text-white focus:outline-none focus:border-primary-500"
              />
            </div>
            <div className="flex flex-col space-y-2">
              <Link href="/" className="px-4 py-3 text-dark-300 hover:text-white hover:bg-white/5 rounded-xl">{t('common.home')}</Link>
              <Link href="/shop" className="px-4 py-3 text-dark-300 hover:text-white hover:bg-white/5 rounded-xl">{t('common.shop')}</Link>
              {isAuthenticated ? (
                <>
                  <Link href="/dashboard" className="px-4 py-3 text-dark-300 hover:text-white hover:bg-white/5 rounded-xl">{t('common.dashboard')}</Link>
                  {isAdmin && <Link href="/admin" className="px-4 py-3 text-dark-300 hover:text-white hover:bg-white/5 rounded-xl">{t('common.admin_panel')}</Link>}
                  <button onClick={logout} className="text-left px-4 py-3 text-rose-400 hover:bg-white/5 rounded-xl">{t('common.logout')}</button>
                </>
              ) : (
                <Link href="/login" className="px-4 py-3 text-primary-400 hover:bg-white/5 rounded-xl font-medium">{t('common.login')}</Link>
              )}
            </div>
          </div>
        </div>
